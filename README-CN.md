...
本仓库收集 Kubernetes 资源清单，[Grafana](http://grafana.com/) 仪表板和通过文案与脚本组合而成的 [Prometheus rules](https://prometheus.io/docs/prometheus/latest/configuration/recording_rules/)，以便使用Prometheus Operator为 [Prometheus](https://prometheus.io/) 提供易于操作的 Kubernetes 集群端到端监控

这个项目的内容是用 [jsonnet](http://jsonnet.org/) 编写的。你既可以将这个项目描述为一个包(package)，也可以将其描述为一个库(library)
...
此堆栈用于监控群集，因此它已预先配置为从所有 Kubernetes 组件收集度量标准。除此之外，它还提供一组默认的仪表板和告警规则。其中许多有用的仪表板和告警参考自 [kubernetes-mixin](https://github.com/kubernetes-monitoring/kubernetes-mixin) 项目，和本项目一样，它提供了可组合的 jsonnet 作为库，供用户根据自己的需求进行定制
...
所有你需要的是一个 Kubernetes 集群，就是这样！ 
我们默认 kubelet 使用令牌身份验证和授权；否则Prometheus需要提供客户端证书，这会使得它有完全权限访问 kubelet，而不仅仅是从 kubelet 获取度量指标，并且令牌认证和授权提供了足够细的粒度和更容易控制的访问
这意味着 kubelet 配置必须包含以下这些flag：

* `--authentication-token-webhook=true` 启用此标志，可以使用一个 ServiceAccount 令牌对 kubelet 进行身份验证.
* `--authorization-mode=Webhook` 启用此标志，kubelet API将使用​​ RBAC 执行请求，以
  确定请求实体（在本例中为Prometheus）是否允许访问资源，本项目中主要是指endpoint为`/metrics`的请求.

此堆栈通过部署[Prometheus Adapter(适配器)](https://github.com/DirectXMan12/k8s-prometheus-adapter/)提供[resource metrics(资源指标)](https://github.com/kubernetes/metrics#resource-metrics-api)。此适配器是Extension API服务器，Kubernetes需要启用此功能，否则适配器无效，但仍然会部署，因此也需要启用以上flag。
